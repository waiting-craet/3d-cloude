...
- 同一对实体（A,B）最多保留一条关系；若有多种关系候选，选择信息量最高的一条并丢弃其余关系。

### 3. 质量约束（非常重要）
1. 优先输出具体、可视化价值高的实体，不要输出“问题/方面/内容/情况/方式/结果”等空泛词。
2. 优先提取名词性实体：专有名词、机构名、系统名、技术名、产品名、事件名、地点名、时间节点。
3. 对同义或近义实体进行归并，只保留一个主名称（例如简称与全称择一）。
4. 若文本较长，请覆盖主要章节中的核心实体，避免只集中在开头段落。
5. 实体名称尽量简洁准确，不要把整句话当作实体。
6. 实体数量不足时，优先补充在文中出现频率更高、且可独立成节点的术语。
7. 关系尽量语义明确，避免大量“相关/关联”等弱关系。
8. 不要输出自环关系（from===to），避免重复关系；同一对实体仅允许一条关系。
9. 为提高3D图谱可读性，请尽量形成多中心连接结构，避免大量孤立点。
10. 目标实体数量约为 ${targetEntityCount}（允许上下浮动），目标关系数量约为 ${targetRelationCount}。
11. 若原文信息充足，请不要只输出少量节点。
