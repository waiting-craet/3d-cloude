...
本功能为导入数据页面添加冗余数据检测能力。当用户点击"生成图谱"按钮导入数据时,系统将检测上传文件中的节点和边是否与当前项目和图谱中已存在的数据冗余,仅导入非冗余数据,并在导入完成后显示冗余数据统计信息。

## 术语表

- **Import_System**: 导入数据处理系统
- **Duplicate_Detector**: 冗余数据检测器
- **Node**: 图谱中的节点实体
- **Edge**: 图谱中的边(关系)
...
3. THE Duplicate_Detector SHALL 基于节点的唯一标识符(如 id 或 label)判断节点是否冗余
4. THE Duplicate_Detector SHALL 基于边的源节点、目标节点和关系类型判断边是否冗余
...
2. WHEN 冗余检测和导入过程进行时, THE Import_Progress_Dialog SHALL 显示冗余节点的数量
3. WHEN 冗余检测和导入过程进行时, THE Import_Progress_Dialog SHALL 显示冗余边的数量
4. THE Import_Progress_Dialog SHALL 显示成功导入的节点数量
5. THE Import_Progress_Dialog SHALL 显示成功导入的边数量
6. THE Import_Progress_Dialog SHALL 以清晰易读的格式展示统计信息
...
3. THE Duplicate_Detector SHALL 使用高效的数据结构(如 Set 或 Map)进行冗余比对
4. THE Import_System SHALL 在检测过程中向用户显示加载状态
...
1. IF 冗余检测过程中发生数据库查询错误, THEN THE Import_System SHALL 显示描述性错误消息
2. IF 冗余检测过程中发生数据格式错误, THEN THE Import_System SHALL 显示描述性错误消息
